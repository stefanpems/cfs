...
Respond by applying ALL these 6 indications to this specific response only: 
...
3. Format the ENTIRE response in HTML including the very first sentence; do not include the doctype tag and the header: write only the content of the HTML body, without the opening and closing "body" tag and DO NOT write any character before and after that HTML content. 
4. In the list of user principal names, replace any occurrence of the domains "mingiustizia.onmicrosoft.com" and "giustizia.it" with the domain "soc.contoso.com"
5. In the list of user principal names, replace any first and family names with generic random English names
6. Translate this ENTIRE response in <RESPONSE_LANGUAGE> from its very first to its very last word. 
```
